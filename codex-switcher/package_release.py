...
def build():
    ...
    target=output/'Codex-Switcher-v2.1.0-Windows-x64.zip';manifest=[]
    ...
    info={'version':'2.1.0','package':target.name,'bytes':target.stat().st_size,'sha256':hashlib.sha256(target.read_bytes()).hexdigest(),'contains_user_data':False,'files':manifest}
