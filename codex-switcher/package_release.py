...
FILES=('codex_switcher.pyw','switcher_core.py','switcher_ui.py','test_switcher.py','test_switcher_ui.py','Launcher.cs','build.py','package_release.py',
       'Codex Switcher.exe','switcher.ico','README.md','TEST_REPORT.md','AGENTS.md','.gitignore',
       'examples/provider.json','examples/codex.toml','examples/provider.env')
def build():
    output=ROOT/'releases';output.mkdir(exist_ok=True)
    target=output/'Codex-Switcher-v2.1.1-Windows-x64.zip';manifest=[]
    ...
    info={'version':'2.1.1','package':target.name,'bytes':target.stat().st_size,'sha256':hashlib.sha256(target.read_bytes()).hexdigest(),'contains_user_data':False,'files':manifest}
