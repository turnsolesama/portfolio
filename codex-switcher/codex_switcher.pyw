...
class App(tk.Tk):
    def __init__(self, ui_scale=None, testing=False):
        ...
        header_size = min((36,45,54,72),key=lambda size:abs(size-px(36)))
        self.brand_image = tk.PhotoImage(file=str(APP_DIR/f'artwork/header-{header_size}.png'))
        self.brand_mark = ttk.Label(head,image=self.brand_image)
        ...
        if DEMO:
            self.environment_label = ttk.Label(head, text='演示环境 · 隔离配置', style='Muted.TLabel')
            self.environment_label.grid(row=0,column=2,rowspan=2,sticky='e')
