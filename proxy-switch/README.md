...
| 使用目标 | 从哪里开始 | 附加条件 |
| --- | --- | --- |
| 切换遵循系统代理的软件 | 添加 HTTP 入口 → 顶部选择目标 → 统一切换 | 已有可用代理客户端或入口 |
| 让入口端口保持不变 | 编辑引擎 → 固定本地入口 → 统一切换 | 已配置并保持运行的受支持引擎 |
| 单独给一个程序分配线路 | 配置引擎后，在程序列表右键指定 | 按 EXE 路径匹配，流量必须已进入引擎 |
| 检查为什么仍走旧线路 | 观察出口与旧连接 → 按需预览重连 | 先区分规则、连接和目标网站的实际可用性 |
