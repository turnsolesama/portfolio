...
2. 打开 **代理管理 → 添加代理**，填写名称、协议、地址和端口。也可以先发现本机候选端口，核对后添加。
3. 在顶部选择直连或已添加的代理，点击 **统一切换**。
4. 刷新网页或重开目标程序，再查看实际连接。需要撤回时点击 **撤回上次更改**。

全新安装的代理列表为空。升级旧版会保留原来的线路名称、端口、路径与稳定标识；某台电脑已有的第三方客户端仅作为该电脑的配置项。
...
「发现本机代理」只读取监听端口与进程路径，找到的端口是候选项，可能是控制端口或其他协议；请在添加窗口核对。它不会自动导入订阅、节点或账号。
...
| 文件 | 用途 |
| --- | --- |
| `config.json` | Version 3 的 Profiles 列表及可选 Routing 引擎 |
...
95 项单元断言覆盖通用代理模型、旧配置迁移、规则优先级、统一切换、失败整体回滚、并发改写保护、Unicode 快捷方式和匿名报告。界面检查读取实际状态、搜索和筛选，不修改网络。
