...
function Get-TcpObservationSnapshot {
    ...
    try{$rows=@(Get-NetTCPConnection -State Listen,Established,SynSent -ErrorAction Stop)}
...
function Get-ApplicationConnectionEvidence($Family,$TcpRows,$CoreConnections,[string]$Gateway,$ById,[string]$PrimaryPath,[bool]$TcpAvailable=$true) {
    $ids=@($Family|ForEach-Object Id);$children=@(Get-ApplicationChildProcesses $Family $PrimaryPath|ForEach-Object Id)
    $counts=@{};$outside=0;$outsidePending=0;$localUnknown=0;$unknown=0;$pending=0;$pendingEndpoints=@{};$childPending=0;$childObserved=0;$total=0
    if($TcpAvailable){foreach($c in $TcpRows){
        ...
        if($entry){if($entry -eq $Gateway){if($engine -and $engine.route){$route=$engine.route}else{$unknown++}}else{$route=$entry}}
...
function Get-ApplicationObservationRow($App,$Family,$Evidence,$Core,$Rule,$Launch,$Identity,[bool]$ProcessesAvailable=$true) {
    ...
    if($Rule){$policy=$Rule.route;$mode='engine';$ruleLoaded=$Rule.loaded -eq $true}
    elseif($Launch){$policy=$Launch.route;$mode='launch'}
    $loaded=$false;$actual=$Evidence.Actual;$observation=$Evidence.State;$repair=$false
    ...
    [pscustomobject]@{Name=$App.Name;Path=$App.Path;SavedPath=$App.SavedPath;RowKey=$App.RowKey;Policy=$policy;PolicyName=$policyName;Mode=$mode;Loaded=$loaded;RuleLoaded=$ruleLoaded;Actual=$actual;Status=$status;PIDs=($ids -join ',');ChildNames=($children -join '、');OutsidePending=$Evidence.OutsidePending;Pending=$Evidence.Pending;ChildPending=$Evidence.ChildPending;ObservationState=$observation;IdentityReason=$reason;RequiresRepair=$repair;CanRepair=$canRepair;Identity=$Identity.Identity;CanLaunch=($mode -eq 'launch' -and -not $repair);HasSavedRule=($mode -ne 'observe');AuthenticationVerified=$false;Coverage='TCP Established/SynSent；连接证据不代表代理握手、目标网站可达或账号登录成功。短时连接、UDP/QUIC 与独立隧道可能不在此快照中'}
}
