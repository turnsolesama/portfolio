...
[void][IO.Directory]::CreateDirectory($qaData);[IO.File]::WriteAllText((Join-Path $qaData 'ui-settings.json'),'{"CloseToTray":false}')
...
$window=$window.Replace('$timer.Start();[Windows.Forms.Application]::Run($form)','$form.Opacity=0;$form.ShowInTaskbar=$false;$timer.Start();[Windows.Forms.Application]::Run($form)')
