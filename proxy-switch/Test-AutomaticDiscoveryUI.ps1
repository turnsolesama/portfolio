...
foreach($name in @('ProxySwitch.ps1','ProxyWindow.ps1','ProxyBackend.ps1','Preferences.ps1','ProxyDiscovery.ps1','ProcessInventory.ps1','AppRouting.ps1','AppRouter.cjs','config.defaults.json')){Copy-Item -LiteralPath (Join-Path $source $name) -Destination $qaRoot}
...
$qaTimer.Add_Tick({
    try{
        ...
        $main=[Windows.Forms.Application]::OpenForms|Where-Object Text -like 'ProxySwitch 3.0.3*'|Select-Object -First 1
        if(-not $main){return}
