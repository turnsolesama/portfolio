...
((root, factory) => {
  ...
  function planTable(plan, category = '', page = 1) {
    ...
    const html = rows.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map(item => `<tr><td><div class="organizer-path" title="${esc(item.source)}">${esc(item.source)}</div><div class="organizer-destination"><span aria-hidden="true">↳</span><span>${esc(item.target||'统一视图 · 保留现有入口')}</span></div></td><td><span class="badge">${esc(categoryLabel(item.category))}</span><p class="organizer-reason">${esc(item.reason)}</p>${item.classification?`<small>${esc(item.classification.scope_label)} · ${esc(item.classification.model_role_label)} · ${esc(item.classification.architecture||'架构待确认')}</small>`:''}</td><td class="organizer-size">${size(item.size)}</td></tr>`).join('');
  ...
  function createPage({api, icon, heading, toast, pollJobs, openModal, closeModal, refresh}) {
    return async function organizerPage(el, params, restored) {
      ...
      try {
        ...
        const virtual=status.policy?.apply_allowed===false||plan?.apply_allowed===false;
        ...
        el.innerHTML = heading('安全区整理', '使用这台电脑的目录，建立可撤销的分类入口。', 'WORKSPACE ORGANIZER') + `
          ...
          <div class="organizer-layout"><section class="panel organizer-setup"><div class="panel-head"><h3>${icon('folder', 17)} 本机安全区</h3><span class="caption-note">01 / SETUP</span></div><div class="body">
            ...
            <label class="organizer-check"><input type="checkbox" id="organizer-startup" ${checked&&!virtual ? 'checked' : ''} ${busy||virtual ? 'disabled' : ''}><span>后台服务启动时自动整理<small>${virtual?'已有模型库使用统一检索视图，不启用启动整理。':'每次服务启动时建立新增文件的分类入口。关闭窗口会保留后台，重开窗口不会重复触发；不添加系统开机项，也不实时监听。'}</small></span></label>
          ...
          </div></section><aside class="panel organizer-rules"><div class="panel-head"><h3>${virtual?'现有模型库分工':'整理方式'}</h3></div><div class="body"><div class="organizer-rule"><span>01</span><div><b>${virtual?'Library · 分类入口':'边界明确'}</b><p>${virtual?'保留现有分类入口，Hub 按文件身份聚合多条兼容路径。':'仅扫描安全区。应用环境、缓存和目录联接等内容会略过。'}</p></div></div><div class="organizer-rule"><span>02</span><div><b>${virtual?'Runtime · 运行路径':'原路径保留'}</b><p>${virtual?'保持应用引用的模型路径，分类只改变检索视图。':'分类目录使用硬链接，共用原文件内容，不复制大模型数据。编辑任意入口也会改变原文件，建议从 Hub 只读浏览。'}</p></div></div><div class="organizer-rule"><span>03</span><div><b>${virtual?'Packages · 完整模型包':'每次都能追溯'}</b><p>${virtual?'保持配置、编码器与分片的配套结构。不拆包，不新建第二棵分类树。':'记录新建和跳过的入口。撤销只移除本次入口，保留原件。'}</p></div></div></div></aside></div>
          <section class="panel organizer-preview"><div class="panel-head"><div><h3>${icon('scan', 17)} 分类预览</h3><span class="caption-note">02 / REVIEW & ORGANIZE</span></div><button class="btn" id="organizer-preview" ${!ready || busy ? 'disabled' : ''}>${icon('scan', 15)} ${plan ? '重新扫描预览' : '扫描并预览'}</button></div>
            ${plan ? `<div class="organizer-stats">${metric('发现文件', summary.scanned, '本次扫描范围')}${metric(virtual?'分类视图条目':'待建立入口', summary.planned, virtual?'仅预览，不创建目录':'执行整份计划')}${metric('已有入口', summary.already_linked, '重复整理自动略过')}${metric('未识别文件', summary.unknown, '已略过，不建立入口')}</div><div class="organizer-plan-info"><span>分类内容逻辑大小 <b>${size(summary.logical_bytes)}</b> · 不等于额外磁盘占用</span><span>另有 ${count(summary.excluded)} 项已排除</span></div>
