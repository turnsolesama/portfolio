...
from . import config, jobs, organizer
...
def preview(cfg):
    ...
    def target():
        ...
        plan = organizer.build_plan(root, config.APP_DIR, snapshot.get("ignore_dirs") or ())
        write_plan(plan)
...
def apply(db, cfg, plan_id):
    ...
    def target():
        with db.asset_lock:
            ...
            result = organizer.apply_plan(plan, storage() / "runs")
