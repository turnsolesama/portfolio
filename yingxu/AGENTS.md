...
- 发布：`python tools/package_release.py`，仅白名单打包；`python tools/verify_release.py releases/YingXu-v0.3.6-Windows-x64.zip` 使用隔离临时目录验证。
...
- 0.3.6 点击项目文件标题复用确认式重命名弹窗；名称输入不含原扩展名，确认时保留扩展名和未保存正文，不逐字自动重命名文件。不可改名来源维持原权限边界。
